...
contract SimpleNft is ERC721A, Ownable, ReentrancyGuard {
    ...
    constructor() ERC721A("SimpleNft", "SNFT") {
        stage = STAGE_CLOSED;
    }
    ...
    ***********************************************************/
    ...
    modifier isValidMerkleProof(bytes32[] calldata merkleProof) {
        require(MerkleProof.verify(merkleProof, _root, 
            keccak256(abi.encodePacked(msg.sender))), "Address not on the list");
        _;
    }
    ...
    ***********************************************************/
    ...
    function devMint(address to, uint256 quantity) external onlyOwner {
        ...
        require(_devMintQuantity + quantity <= MAX_DEV_MINT, "Dev mint quantity exceeded");
     ...
     */
    function whitelistMint(uint256 quantity, bytes32[] calldata merkleProof) external payable nonReentrant isValidMerkleProof(merkleProof) {
    ...
    */
     ...
     */
    function setWhitelistMintParams(uint256 newPrice, uint256 newMaxMint) external onlyOwner {
     ...
     */
    function setPublicMintParams(uint256 newPrice, uint256 newMaxMint) external onlyOwner {
        require(stage == STAGE_CLOSED, "Invalid stage");
